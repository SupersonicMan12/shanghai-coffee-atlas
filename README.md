...
- **Ten kinds of room.** Each café is drawn as a glyph for its archetype — a roasting drum,
  ...
  anything.
- **From where you stand.** Your location by default (or a metro station / a tap on the
  map); results are ranked by walking minutes and only cafés open at the hour on the time
  bar are shown.
...
- **Time of day.** Five palettes from first light to after dark; drag the time bar and the
  sheet relights and closed cafés drop out of the ranking.
- **Shareable.** Café, anchor and compass state live in the URL hash.
