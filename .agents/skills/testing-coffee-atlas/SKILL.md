...
- Unverified editorial cafés (src/data/unverified.json) render `span.verdict-est` (未核实 / unverified) next to the headline and `.strip-card.sketch` in the strip; `title` holds the tooltip text.
- v5+ (Compass/Passport only, no onboarding dialog): passport key `shca.passport.v1`. Location panel selectors: `.loc-me` (`.on` / `.busy`), `.loc-note`, `.loc-anchor`, `.loc-hits`; user marker `g.me` / `.me-pulse` / `text.me-label`. Hard filters `.section-limits .section-toggle em.count`. Empty state `.strip-empty`. Header time slider `input[type=range][aria-label="时间"]` (0–24 h, step 0.5).
- Geolocation emulation: `/tmp/cdp_geo.py grant <lng> <lat> | deny | prompt` (CDP `Browser.setPermission` + `Emulation.setGeolocationOverride`). Permission grants may not stick across a fresh profile — if a native Chrome bubble appears on 「用我的位置」 click, that itself is evidence of the "prompt" state; click "Allow while visiting the site" to continue. Success sets the `me` anchor (已定位到你) and writes **no** `at=` to the hash — only metro/pin anchors are carried in the URL; on load, if permission is already granted, the app re-locates silently (out-of-range/denied shows no red note until the user taps). Test out-of-range with `grant 116.40 39.90`.
- Mobile 390×844: the LocationPanel is folded into one row in the peek strip (`.sheet .location`, above the scenario chips, visible at `snap-peek`); `focusOn` centres in the stage area above the sheet (reads `--sheet-peek`); locating / picking an anchor / arming the map pin drops the sheet to `snap-peek` so the fly-to and marker are visible. Drag the grip in several `mouse_move` steps (a single `left_click_drag` often does not reach `snap-full`). Check `.sheet` class (`snap-peek|half|full|hidden`) via console.
